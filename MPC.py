...
class MPC:
    def __init__(self, params):
            ...
            params
                ...
                .opt_cfg
                    ...
                    .percentile (float): The percentile used for either catastrophic state or reward-based
                        risk aversion.
                    .penalty_scale (float): The scaling factor to deviate from the original penalty used during
                        catastrophic state risk aversion (new parameter).
        """
        self.dO, self.dU = (
        ...
        self.catastrophe_cost_fn = get_required_argument(
            params.opt_cfg, "catastrophe_cost_fn", "Must provide cost on catastrophe."
        )
        self.no_catastrophe_pred = params.opt_cfg.get("no_catastrophe_pred")
        self.percentile = get_required_argument(
            params.opt_cfg, "percentile", "Must provide percentile used for optimizer"
        )
        # add penalty scale, default to 1
        self.penalty_scale = params.prop_cfg.get("penalty_scale", 1)
        ...
        print(
            "Created an MPC controller, prop mode %s, %d particles. "
    ...
    def train(self, obs_trajs, acs_trajs, rews_trajs, gravity_vals=None):
        ...
        idxs = np.random.randint(
            self.train_in.shape[0], size=[self.model.num_nets, self.train_in.shape[0]]
        )
        ...
        batch_size = (
            256
            if "batch_size" not in self.model_train_cfg
            else self.model_train_cfg["batch_size"]
        )
        ...
        for _ in epoch_range:
            ...
            for batch_num in range(num_batch):
                ...
                train_in = (
                    torch.from_numpy(self.train_in[batch_idxs]).to(TORCH_DEVICE).float()
                )
                train_targ = (
                    torch.from_numpy(self.train_targs[batch_idxs])
                    .to(TORCH_DEVICE)
                    .float()
                )
                ...
                if not self.no_catastrophe_pred:
                    ...
                    if num_catastrophes == 0:
                        ...
                        pos_weight = (catastrophe_targ.numel() - num_catastrophes).to(
                            dtype=torch.float
                        ) / num_catastrophes
                    catastrophe_loss = torch.nn.BCEWithLogitsLoss(pos_weight)(
                        catastrophe_prob, catastrophe_targ
                    )
            ...
            with torch.no_grad():
                val_in = (
                    torch.from_numpy(self.train_in[idxs[:, :5000]])
                    .to(TORCH_DEVICE)
                    .float()
                )
                val_targ = (
                    torch.from_numpy(self.train_targs[idxs[:, :5000]])
                    .to(TORCH_DEVICE)
                    .float()
                )
                ...
                if not self.no_catastrophe_pred:
                    ...
                    if num_catastrophes == 0:
                        ...
                        pos_weight = (
                            val_catastrophe_targ.numel() - num_catastrophes
                        ).to(dtype=torch.float) / num_catastrophes
                    catastrophe_loss = torch.nn.BCEWithLogitsLoss(pos_weight)(
                        catastrophe_prob, val_catastrophe_targ
                    )
            ...
            epoch_range.set_postfix(
                {
                    ...
                    "Catastrophe pred loss": catastrophe_loss
                    if not self.no_catastrophe_pred
                    else 0,
                }
            )
        ...
        if d_random or d_cem:
            if not self.has_been_trained:
                return self.possible_actions[
                    np.random.choice(
                        np.arange(self.possible_actions.shape[-1]), size=1
                    )[0]
                ]
            ...
            if d_random:
                ...
                self.prev_sol = np.concatenate(
                    [np.copy(soln)[1:], np.zeros((1, self.per * self.dU))]
                )
    ...
    def _compile_cost(self, ac_seqs):
        ...
        for t in range(self.plan_hor):
            ...
            if self.mode == "test" and not self.no_catastrophe_pred:
                # use catastrophe prediction during adaptation
                # new: scale by penalty_scale
                cost = self.catastrophe_cost_fn(
                    next_obs, cost, self.percentile, self.penalty_scale
                )
        ...
        if self.no_catastrophe_pred:
            ...
            if self.percentile <= 100:
                k_percentile = -(-costs).kthvalue(
                    k=max(int((self.percentile / 100) * costs.shape[1]), 1), dim=1
                )[0]
                cost_mask = costs < k_percentile.view(-1, 1).repeat(1, costs.shape[1])
            else:
                k_percentile = costs.kthvalue(
                    k=max(int(((200 - self.percentile) / 100) * costs.shape[1]), 1),
                    dim=1,
                )[0]
                cost_mask = costs > k_percentile.view(-1, 1).repeat(1, costs.shape[1])
            costs[cost_mask] = 0
